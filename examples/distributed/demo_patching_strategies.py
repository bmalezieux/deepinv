r"""
Distributed Image Patching Strategies
======================================

This example demonstrates two patching strategies for distributed processing of large images.
The distributed framework enables processing gigapixel images by automatically dividing them into patches,
processing patches independently or in parallel, and reconstructing the final result.

This example covers:

- :func:`deepinv.distributed.strategies.BasicStrategy`: Simple non-overlapping tiling (fast but can create boundary artifacts)
- :func:`deepinv.distributed.strategies.OverlapTilingStrategy`: Overlapping patches with padding for artifact-free reconstruction
...
def plot_patches(ax, image, strategy, color, show_padding=False):
    ...
    if show_padding:
        ...
        img_display = np.pad(img_np, ((pads[2], pads[3]), (pads[0], pads[1]), (0, 0)),
            mode=getattr(strategy, "pad_mode", "reflect"))
        h, w = img_np.shape[:2]
        # Draw rectangles for: original image boundary and full padded image boundary
        ax.add_patch(Rectangle((pads[0], pads[2]), w, h, linewidth=2.5, edgecolor="red",
            facecolor="none", linestyle="--", path_effects=halo))
        ax.add_patch(Rectangle((0, 0), *img_display.shape[:2][::-1], linewidth=2.5, edgecolor="black",
            facecolor="none", linestyle=":", path_effects=[pe.Stroke(linewidth=3.5, foreground="white"), pe.Normal()]))
        ax.legend(handles=[Patch(facecolor="none", edgecolor="red", linestyle="--", linewidth=2, label="Original image"),
            Patch(facecolor="none", edgecolor="black", linestyle=":", linewidth=2, label="Image with global padding"),], 
    ...
    slices = strategy._patch_slices if isinstance(strategy, BasicStrategy) else strategy._global_slices
    # Draw patches with highlighting on patch 0 (first/reference patch)
    for idx, slc in enumerate(slices if isinstance(slices, list) else [slices[i] for i in range(len(slices))]):
        h_slice, w_slice = (slc[-2], slc[-1]) if isinstance(slices, list) else (strategy._patch_slices[idx][-2], strategy._patch_slices[idx][-1])
        is_highlight = idx == 0  # Highlight first patch for reference
        edge = color if is_highlight else ("salmon" if isinstance(strategy, BasicStrategy) else "skyblue")
        # Draw rectangle for each patch with thicker outline and halo for highlighted patch
        ax.add_patch(Rectangle((w_slice.start, h_slice.start), w_slice.stop - w_slice.start,
            h_slice.stop - h_slice.start, linewidth=2.5 if is_highlight else 1.2,
            edgecolor=edge, facecolor="none", path_effects=halo if is_highlight else None))        
        ax.text(w_slice.start + 6, h_slice.start + 16, f"patch {idx}", color="black",
            fontsize=12 if is_highlight else 11, weight="bold" if is_highlight else "normal",
            bbox=dict(boxstyle="round,pad=0.3", facecolor="white", edgecolor=edge, linewidth=0.8))
...
ax.set_title("OverlapTilingStrategy: Overlapping Patches with Padding", fontsize=12, fontweight="bold")
ax.axis("off")
plt.show()
...
def plot_single_patch(ax, image, strategy, patch_idx, color):
    ...
    ax.add_patch(Rectangle((0, 0), w, h, linewidth=3, edgecolor=color, facecolor="none",
        path_effects=[pe.Stroke(linewidth=4.5, foreground="white"), pe.Normal()]))
    ax.text(8, 20, f"patch {idx}", color="black", fontsize=12, weight="bold",
...
overlap_h = overlap_strategy._metadata['window_shape'][0]  # Height of overlap patch (includes receptive field)
...
ax0.set_title(f"BasicStrategy\nPatch {patch_idx} ({patch_size}×{patch_size})", fontsize=12, fontweight="bold", pad=10)
...
ax1.set_title(f"OverlapTilingStrategy\nPatch {patch_idx} ({overlap_h}×{overlap_h})", fontsize=12, fontweight="bold", pad=10)
#plt.tight_layout()
plt.show()
...
print(f"OverlapTilingStrategy: {overlap_strategy.get_num_patches()} patches, stride={patch_size}, overlap={overlap}, Patch: {overlap_strategy._metadata['window_shape']}")
print("=" * 60)
